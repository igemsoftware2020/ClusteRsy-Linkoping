...
mod_enrichDGNv_ui <- function(id){
  ns <- NS(id)
  tagList(
    uiOutput(ns("module_input")),
    uiOutput(ns("error_p_value")),
    
    #Description of the method "Enrichment analysis based on snp-gene-disease associations from several public data sources and the literature"
...
#' enrichDGNv Server Function
#'
#' @noRd 
# con should ne somewhere in the code?
mod_enrichDGNv_server <- function(input, output, session, con){
  ns <- session$ns
  ...
  observeEvent(input$load_inputDO, {
    ...
    enrichment_objectONE <- try(DOSE::enrichDGNv(gene = module_genes,
...
# mod_DOSE_ui("enrichDGNv_ui_1")

## To be copied in the server
# callModule(mod_enrichDGNv_server, "enrich_ui_1")