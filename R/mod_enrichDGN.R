...
mod_enrichDGN_server <- function(input, output, session, con, Description1_ui_1, module_overview_ui_1){
  ...
  output$module_input <- renderUI({
    ...
    tagList(
      ...
      tags$div(id = "error_name_enrichDGN_js",
  ...
  observe({
    if (any(MODifieRDB::get_available_enrichment_objects(con)$enrichment_name == enrichment_name())){
      output$error_name_js <- renderUI({
        tags$script(HTML("element = document.getElementById('error_name_enrichDGN_js');
                       element.classList.add('has-error');
                       document.getElementById('main_page_v2_ui_1-Columns_ui_1-disease_analysis_ui_1-enrichDGN_ui_1-load_input').disabled = true;"))
      })
      ...
      output$error_name_js <- renderUI({
        tags$script(HTML("document.getElementById('error_name_enrichDGN_js').classList.remove('has-error');
                         document.getElementById('main_page_v2_ui_1-Columns_ui_1-disease_analysis_ui_1-enrichDGN_ui_1-load_input').disabled = false;"))
      })
